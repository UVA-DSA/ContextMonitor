...
class injectFaults:
    ...
    """
    marginal addition over previous value in x-y-z
    """
    def cart_errorModel(self, c_data, _start, _end, err):

        e_data = c_data
        scale_factor = err
        e_count = 0.0
        l_max = np.amax(c_data[:,0])
        m_max = np.amax(c_data[:,1])
        r_max = np.amax(c_data[:,2])

        checkpoint_l = c_data[_start][0]
        checkpoint_m = c_data[_start][1]
        checkpoint_r = c_data[_start][2]
        next_checkpoint = c_data[_end]
        transc, thresh = self.printMax(c_data)
        for i in range(_start, _end):
            checkpoint_l =  scale_factor*l_max
            checkpoint_r =  scale_factor*r_max
            checkpoint_m =  scale_factor*m_max
            e_data[i][0] = c_data[i][0] + checkpoint_l
            e_data[i][1] = c_data[i][1] + checkpoint_m
            e_data[i][2] = c_data[i][2] + checkpoint_r

        for i in range(len(transc)):
            e_count = 0
            for j in range(transc[i][0], transc[i][1]):
                if e_data[j][2] <thresh[i][5] or e_data[j][0] > thresh[i][0] or e_data[j][0] < thresh[i][1] or e_data[j][1] > thresh[i][2] or e_data[j][1] < thresh[i][3] or e_data[j][2] > thresh[i][4]:
                    e_count = e_count+1

            print "threshold exceeded in segment {} for {} times".format(transc[i][2], e_count)

        return e_data, scale_factor
    def mfi_code(self, traj, _start, _end, err):
        ...
        return traj
    def printMax(self, c_data):
        transc = externals.joblib.load('{}/transition_points1.p'.format(self.picklePath))
        thresh = []
        for i in range(len(transc)):
            l_max = np.amax(c_data[transc[i][0]:transc[i][1],0])
            l_min = np.amin(c_data[transc[i][0]:transc[i][1],0])

            m_max = np.amax(c_data[transc[i][0]:transc[i][1],1])
            m_min = np.amin(c_data[transc[i][0]:transc[i][1],1])

            r_max = np.amax(c_data[transc[i][0]:transc[i][1],2])
            r_min = np.amin(c_data[transc[i][0]:transc[i][1],2])

            thresh.append([l_max,l_min, m_max, m_min, r_max, r_min])
        return transc, thresh
    ...
    def plotgrasp(self, e_grasp, grasp, gscale_factor, tscale_factor, _dir):
        ...
        #51064-52063
        plt.axvline(26705, color = 'b', linestyle = ':')
        plt.axvline(38426, color = 'b', linestyle = ':')
    ...
    def plotCart(self, gscale_factor, tscale_factor, left_z, right_z, expDir):
        ...
        plt.xlabel(x_label, fontsize = 35)
        plt.ylabel(y_label, fontsize = 35)
        plt.axvline(38460, color = 'b', linestyle = ':')
        plt.annotate('Fault detected', xy=(38460, np.max(right_z)), xytext=(15776, 0.7*np.max(right_z)),arrowprops=dict(arrowstyle="->", shrinkA=0, shrinkB=10, facecolor='black'), fontsize = 40)
        plt.axvline(38590, color = 'b', linestyle = ':')
        plt.annotate('Failure', xy=(38590, np.max(right_z)), xytext=(48477, 0.7*np.max(right_z)),arrowprops=dict(arrowstyle="->", shrinkA=0, shrinkB=10, facecolor='black'), fontsize = 40)
